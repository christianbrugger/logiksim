...
auto add_random_wire(Rng& rng, State state, grid_t min, grid_t max, bool random_modes) {
    const auto line = get_random_line(rng, min.value, max.value);
...
auto add_many_wires(Rng& rng, State state, bool random_modes, int max_tries) -> void {
    ...
    for (auto _ [[maybe_unused]] : range(tries)) {
        add_random_wire(rng, state, min, max, random_modes);
    }
}

auto add_many_wires_and_buttons(Rng& rng, State state, bool random_modes, int max_tries)
    -> void {
    const auto min = grid_t {5};
    const auto max = grid_t {10};
    ...
    for (auto _ [[maybe_unused]] : range(tries)) {
        if (get_random_bool(rng, 0.1)) {
            add_random_button(rng, state, min, max, random_modes);
        } else {
            add_random_wire(rng, state, min, max, random_modes);
        }
    }
