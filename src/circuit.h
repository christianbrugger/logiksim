...
public:
		...
		connection_size_t output_count)
	{
		...
		inputs_.resize(inputs_.size() + input_count - 1);
		outputs_.resize(outputs_.size() + output_count - 1);
	...
	{
		const Element& from_node = get_element_node(from_element);
		Connectivity& from_conn = get_output_node(from_node, from_output);

		const Element& to_node = get_element_node(to_element);
		Connectivity& to_conn = get_input_node(to_node, to_input);
	...
	element_size_t get_output_element(
		[[maybe_unused]] element_size_t element, 
		[[maybe_unused]] connection_size_t output)
	{
		return null_element;
	}
