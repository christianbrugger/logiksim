...
auto get_random_grid(Rng& rng, grid_t min = grid_t::min(), grid_t max = grid_t::max())
    -> grid_t;
auto get_random_point(Rng& rng, grid_t min = grid_t::min(), grid_t max = grid_t::max())
    -> point_t;
auto get_random_point(Rng& rng, rect_t rect) -> point_t;

auto get_random_line(Rng& rng, grid_t min = grid_t::min(), grid_t max = grid_t::max())
    -> ordered_line_t;
auto get_random_line(Rng& rng, grid_t min, grid_t max, grid_t max_length)
    -> ordered_line_t;

auto get_random_lines(Rng& rng, std::size_t count, grid_t min = grid_t::min(),
                      grid_t max = grid_t::max()) -> std::vector<ordered_line_t>;
