...
auto _replay_last_entry(CircuitData& circuit, HistoryStack& stack) -> void {
    switch (stack.top_entry().value()) {
        ...
        case segment_set_endpoints: {
            ...
            const auto segment = to_id(segment_key, circuit);
            editing::set_temporary_endpoints_with_history(circuit, segment, endpoints);
            return;
        }
        ...
        case segment_split: {
            ...
            Expects(definition.new_key);

            if (is_inserted(source_segment.wire_id)) {
                editing::split_inserted_segment(
